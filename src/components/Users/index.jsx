                ...
                {list.map((item, i) => {
                    return (
                        <div className={"col-span-12 grid grid-cols-12"} key={item.id}>
                            <div
                                className="col-span-6 sm:col-span-6 flex justify-between items-center text-grey-dark ctext-sm  border-b border-grey-border py-4 px-2">
                                <div className="cursor-pointer hover:text-blue">
                                    {item.name}
                                </div>
                            </div>
                            <div
                                className="col-span-6 sm:col-span-3 flex justify-end sm:justify-start items-center text-grey-dark ctext-sm  border-b border-grey-border py-4 px-2">
                                {item.link}
                            </div>
                            <div
                                className="col-span-12 sm:col-span-3 flex justify-start sm:justify-end items-center border-b border-grey-border  py-4 px-2 mb-8 sm:mb-0">
                                <div onClick={() => setChange(true)}
                                     className="text-blue ctext-base font-medium underline cursor-pointer hover:text-green-light">Изменить
                                    пароль
                                </div>
                            </div>
                        </div>
